...
const Home = () => {
    ...
    const ai = new GoogleGenAI({
        apiKey: "AIzaSyDbaXbk8TCX87u1iwg46TdYY8v-sjmf-cs"
    });
    ...
    const getResponse = async () => {
        if (!prompt.trim()) alert("Please describe your component first");
        ...
        <>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 px-6 lg:px-16">
                <div className="w-full py-6 rounded-xl bg-[#141319] mt-5 p-5">
                    <h3 className='text-[25px] text-purple-600  font-semibold '>AI Component Generator</h3>
                    <p className='text-gray-400 mt-2 text-[16px]'>Describe your component and let AI code it for you.</p>

                    <p className='text-[15px] text-white font-[700] mt-4'>Framework</p>
                    <Select
                        className='mt-2'
                        options={options}
                        value={frameWork}
                    ...
                    <p className='text-[15px] text-white  font-[700] mt-5'>Describe your component</p>
                    <textarea
                    ...
                    <div className="flex items-center justify-between mt-3">
                        ...
                        >
                            {loading ? <CgSpinner color='white' size={18}  className='animate-spin'/> : <BsStars />}
                            Generate
                        </button>
                    </div>
                </div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        {tab === 1 ? (
                                            <>
                                                ...
                                                onClick={copyCode} className="w-10 h-10 rounded-xl border border-zinc-800 flex text-purple-600 items-center justify-center hover:bg-[#333]"><IoCopy /></button>
                                                <button title='Download code'
                                                onClick={downnloadFile} className="w-10 h-10 rounded-xl border border-zinc-800 flex text-purple-600 items-center justify-center hover:bg-[#333]"><PiExportBold /></button>
                                            </>
                                        ) : (
                                            <>
                                                <button title='Open in New Tab'
                                                onClick={() => setIsNewTabOpen(true)} className="w-10 h-10 rounded-xl border text-purple-600 border-zinc-800 flex items-center justify-center hover:bg-[#333]"><ImNewTab /></button>
                                                <button title='Refresh Preview'
                                                onClick={() => setRefreshKey(prev => prev + 1)} className="w-10 h-10 rounded-xl text-purple-600 border border-zinc-800 flex items-center justify-center hover:bg-[#333]"><FiRefreshCcw /></button>
                                            </>
                                        )}
                                    </div>
                                </div>
